                                     ...
                                     const ComPtr<ID3D12Device>& device, const ComPtr<ID3D12CommandQueue>& commandQueue)
{
  ...
  throwIfFailed(device->CreateCommittedResource(&heapProperties, D3D12_HEAP_FLAG_NONE, &textureDesc,
                                                D3D12_RESOURCE_STATE_COPY_DEST, nullptr,
                                                IID_PPV_ARGS(&textureResource)));

  UploadHelper uploadHelper(device, GetRequiredIntermediateSize(textureResource.Get(), 0, 1));
  uploadHelper.uploadTexture(const_cast<void*>(data), textureResource, textureWidth, textureHeight, commandQueue);
                                         ...
                                         const ComPtr<ID3D12DescriptorHeap>& descriptorHeap, i32 descriptorIndex) const
{
  ...
  CD3DX12_CPU_DESCRIPTOR_HANDLE srvHandle(
      descriptorHeap->GetCPUDescriptorHandleForHeapStart(), descriptorIndex,
      device->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV));

  device->CreateShaderResourceView(m_textureResource.Get(), &shaderResourceViewDesc, srvHandle);
}
} // namespace gims