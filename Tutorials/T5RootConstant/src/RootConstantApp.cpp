...
private:
  ...
  void createPipeline()
  {
    m_triangleMeshProgram =
        HLSLProgram(L"../../../Tutorials/T5RootConstant/Shaders/RootConstant.hlsl", "VS_main", "PS_main");
...
public:
  ...
  virtual void onDraw()
  {
    ...
    m_counter++;
    f32 t     = f32(m_counter) / 30.0f;
    f32 f_val = 2.0f + sin(0.5f * t);

    // const auto f = std::bit_cast<ui32, f32>(0.5f * (sin(f32(m_counter) / 60.0f) + 2.0f));

    commandList->SetGraphicsRoot32BitConstant(0, std::bit_cast<ui32, f32>(f_val), 0);
...
int main(int /* argc*/, char /* **argv */)
{
  ...
  config.title    = L"Tutorial 5 Root Constants";
